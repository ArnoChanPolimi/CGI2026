...
CSV_PATH = (
    ...
    / "output"
    / "dataset_index"
    / "dataset_index_letterbox_NoHealthy_v1_NoAloeVera.csv"
)  # <--- 对齐新 CSV 名称

# SAVE_DIR = ROOT_DIR / "output" / "All_crop_train_logic_v1"
SAVE_DIR = ROOT_DIR / "output" / "All_crop_train_logic_v1" / "pth"  # <--- 专门存 pth
...
# )
# 2. 训练日志路径：修改到 log 文件夹下
LOG_DIR = ROOT_DIR / "log" / "train" / "All_crop_train_logic_v1"
LOG_DIR.mkdir(parents=True, exist_ok=True)  # 确保创建多级目录

# 修改 logger 初始化，指向新的 LOG_DIR
logger = get_logger(
    log_dir=str(LOG_DIR), log_filename="logic_v1_training_NoHealthy_NoAloeVera.log"
)
# ===============================================
...
def train():
    ...
    # 1. 自动扫描最新的 pth 文件
    all_pths = list(SAVE_DIR.glob("checkpoint_epoch_letterbox_NoAloeVera_*.pth"))
    latest_epoch = 0
    latest_pth = None
    ...
    if all_pths:
        try:
            ...
            epochs = [
                int(
                    re.findall(
                        r"checkpoint_epoch_letterbox_NoAloeVera_(\d+).pth", p.name
                    )[0]
                )
                for p in all_pths
            ]
            ...
            latest_pth = (
                SAVE_DIR / f"checkpoint_epoch_letterbox_NoAloeVera_{latest_epoch}.pth"
            )
    ...
    # 在 train() 函数里加入
    logger.info(f"📊 类别分布统计: Biotic={counts[1]}, Abiotic={counts[0]}")
    ...
    for epoch in range(start_from, EPOCHS):
        ...
        save_path = SAVE_DIR / f"checkpoint_epoch_letterbox_NoAloeVera_{epoch+1}.pth"
