...
ERROR_CSV_FILE = ERROR_OUT_DIR / f"all_misclassified_cases_{ver_tag}_letterbox.csv"
CROP_REPORT_FILE = ERROR_OUT_DIR / f"crop_error_diagnosis_{ver_tag}_letterbox.csv"

# --- 📁 B路：图像输出目录 (VIS_OUT_DIR 已定义为 SAVE_DIR / "output") ---
# 确保这里使用了 VIS_OUT_DIR，让混淆矩阵和 Loss 图各就各位
LOSS_ACC_PLOT_PATH = VIS_OUT_DIR / f"loss_Acc_logic_v1_letterbox.png"
CM_PLOT_PATH = VIS_OUT_DIR / f"confusion_matrix_logic_v1_{ver_tag}_letterbox.png"
...
logger = get_logger(
    log_dir=str(LOG_OUT_DIR), log_filename=f"logic_v1_analysis_{ver_tag}_letterbox.log"
)
...
def analyze():
    ...
    METRICS_CSV = VIS_OUT_DIR / "backtrack_metrics_logic_v1.csv"
    ...
    if METRICS_CSV.exists():
        logger.info(f"📂 发现已存在的测算记录: {METRICS_CSV.name}，正在秒速读取数据...")
    ...
    if not train_acc_list:
        logger.warning("🧪 未发现完整测算记录，开始 1-12 轮全量考试 (请确保 GPU 可用)...")
        ...
        back_loader = DataLoader(back_ds, batch_size=256, shuffle=False, num_workers=4)

        train_ds_back = PlantDataset(CSV_PATH, ROOT_DIR, split="train", transform=tmp_tf)
        train_loader_back = DataLoader(train_ds_back, batch_size=256, shuffle=False, num_workers=4)
        ...
        for e in pbar_epoch:
            ...
            if pth.exists():
                tmp_model.load_state_dict(torch.load(pth, map_location=CURRENT_DEVICE))
                tmp_model.eval()
                ...
                with torch.no_grad():
                    for imgs, lbls in back_loader:
                        ...
                        t_loss += back_criterion(outs, lbls.to(CURRENT_DEVICE)).item()
                        t_corr += (torch.argmax(outs, 1) == lbls.to(CURRENT_DEVICE)).sum().item()
                        t_total += lbls.size(0)
                ...
                with torch.no_grad():
                    for imgs, lbls in train_loader_back:
                        outs = tmp_model(imgs.to(CURRENT_DEVICE))
                        tr_corr += (torch.argmax(outs, 1) == lbls.to(CURRENT_DEVICE)).sum().item()
                        tr_total += lbls.size(0)
                train_acc_list.append(tr_corr / tr_total)
            else:
                test_losses_backtrack.append(None); test_accs_backtrack.append(None); train_acc_list.append(None)
    ...
    plt.subplot(2, 1, 2)
    plt.plot(epochs, train_acc_list, marker="o", color="#f1c40f", linewidth=2, label="Train Acc")
    plt.plot(epochs, test_accs_backtrack, marker="D", color="#9b59b6", linewidth=2, label="Test Acc")
    for i, v in zip(epochs, test_accs_backtrack):
        if v is not None: plt.text(i, v + 0.01, f"{v:.2f}", ha="center", fontweight="bold")
