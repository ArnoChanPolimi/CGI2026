...
def analyze():
    logger.info("--- 🔍 开始 Logic_V1 全方位评估报告 ---")
    ...
    try:
        ...
        plt.subplot(1, 2, 1)
        plt.plot(step_losses, color="#3498db", alpha=0.3, label="Batch Loss")
        if len(step_losses) > 50:
            plt.plot(
                pd.Series(step_losses).rolling(window=20).mean(),
                color="#2980b9",
                label="Smoothed",
            )
        plt.title("Training Loss (Logic V1)")
        ...
        plt.savefig(LOSS_PLOT_PATH)  # 保存到 All_crop_train_logic_v1
        logger.info(f"✅ 趋势图已保存至: {LOSS_PLOT_PATH.name}")
