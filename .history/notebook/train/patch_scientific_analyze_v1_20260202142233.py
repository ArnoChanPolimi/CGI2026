...
def sync_and_analyze():
    ...
    # ========================== F. 额外输出：最新一轮的混淆矩阵 ==========================
    # 使用最后一次循环中留下的 labs 和 preds
    if "labs" in locals() and "preds" in locals():
        ...
        plt.title(f"Confusion Matrix (Latest Epoch)")
        plt.ylabel("Actual")
        plt.xlabel("Predicted")
        plt.savefig(SAVE_DIR / "confusion_matrix_latest.png")
        plt.close()  # 记得关闭，否则占用内存
        print(f"3. 混淆矩阵：confusion_matrix_latest.png")

    print(f"🏁 分析完成！路径: {SAVE_DIR}")
