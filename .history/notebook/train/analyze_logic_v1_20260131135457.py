    ...
    try:
        ...
        for e in range(1, len(epoch_avg_losses) + 1):
            ...
            if pth.exists():
                ...
                test_losses_backtrack.append(None); test_accs_backtrack.append(None)
        ...
        plt.plot(epochs, epoch_avg_losses, marker="o", color="#e74c3c", linewidth=2, label="Train Loss")
        plt.plot(epochs, test_losses_backtrack, marker="s", color="#3498db", linewidth=2, label="Test Loss")
        
        # 你的数值标注循环 (保留！)
        for i, v in zip(epochs, epoch_avg_losses):
            plt.text(i, v + (max(epoch_avg_losses) * 0.01), f"{v:.4f}", ha="center", fontweight="bold")
        ...
        plt.subplot(2, 1, 2)
        if train_acc_list:
            plt.plot(epochs, train_acc_list, marker="o", color="#f1c40f", label="Train Acc")
        plt.plot(epochs, test_accs_backtrack, marker="D", color="#9b59b6", label="Test Acc")
