...
# ================= 配置区 =================
# 获取当前脚本所在位置 (notebook/preprocess/your_script.py)
current_file = Path(__file__).resolve()

# 向上跳三级找到项目根目录 (preprocess -> notebook -> CGI_PROJECT)
ROOT_DIR = current_file.parent.parent.parent

DATA_DIR_NAME = "RAW_DATA"  # 原始数据文件夹名

# --- 新的输出逻辑 ---
# 目标文件夹：根目录/output/dataset_index
OUTPUT_FOLDER = ROOT_DIR / "output" / "dataset_index"
# 如果文件夹不存在，则自动创建 (parents=True 表示连带创建 output 文件夹)
...
# 为了兼容你后续的 os.path 逻辑，将 Path 对象转为字符串
