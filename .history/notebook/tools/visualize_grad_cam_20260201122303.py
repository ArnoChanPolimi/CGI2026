...
# ========================== 核心修复：路径补全 ==========================
# 1. 强制获取项目根目录 (CGI_project)
current_file = Path(__file__).resolve()
ROOT_DIR = current_file.parent.parent.parent  # 向上跳三级：tools -> notebook -> ROOT
sys.path.append(str(ROOT_DIR))  # 把根目录塞进 Python 的搜索名单

# 2. 现在可以“认真”导入了
try:
    # 尝试导入，如果还是不行就手动指定 ROOT_DIR
    from notebook.test.cross_species_benchmarking_pth12 import ROOT_DIR
except (ImportError, ModuleNotFoundError):
    print(f"⚠️ 自动导入失败，已手动锁定根目录: {ROOT_DIR}")

from train.model import get_plant_model  # 直接从 train 导入
...
def main():
    ...
    for rel_path in tqdm(images_to_test):
        ...
        axes[2].imshow(get_res(mask16, base_img_rgb))
        axes[2].set_title("Layer 16 (Disease)")
