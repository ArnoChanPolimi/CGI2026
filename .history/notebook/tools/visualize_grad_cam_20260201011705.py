...
class GradCAM:
    def __init__(self, model, target_layer):
        ...
        self.gradients = None
        # 只挂 forward_hook 拿特征图
        self.handler = target_layer.register_forward_hook(self._save_feature_maps)

    def _save_feature_maps(self, module, input, output):
        self.feature_maps = output.detach()

        # 【核心修正】：直接在输出 Tensor 上挂 hook 拿梯度，避开 Module 级别的 hook 警告
        def save_grad(grad):
    ...
    def release(self):
        self.handler.remove()  # 移除 forward hook 即可
