...
RESULT_DIR = (
    ...
    / "All_crop_train_logic_v1"
    / "cross_species_test"
    / "NohealthyNoAloeVera_order_test"
    / ver_tag
)
...
logger = get_logger(
    log_dir=str(LOG_DIR),
    log_filename=f"cross_species_benchmarking_{ver_tag}_NoHealthy_NoAloeVera_order.log",
)
...
# ========================== 2. 加载模型与配置 ==========================
logger.info("=" * 50)
logger.info(f"🏆 跨物种实战测试最终报告 ({ver_tag})")
...
def save_confusion_matrix(y_true, y_pred, species_name):
    ...
    # 修改后：
    save_path = (
        RESULT_DIR / f"cm_{suffix}_{ver_tag}_letterbox_NoHealthy_NoAloeVera_order.png"
    )
...
def run_benchmark():
    ...
    for species_folder in species_folders:
        ...
        if y_true:
            ...
            df_species.to_csv(
                # RESULT_DIR / f"detail_{species_name}_pth12_letterbox.csv", # 不要使用硬编码
                # 修改后 (使用 ver_tag)
                RESULT_DIR / f"detail_{species_name}_{ver_tag}_letterbox.csv",
                index=False,
                encoding="utf-8-sig",
            )
    ...
    if all_summary:
        ...
        summary_df.to_csv(
            # 修改后 (使用 ver_tag)
            RESULT_DIR
            / f"cross_species_summary_{ver_tag}_letterbox_NoHealthy_NoAloeVera_order.csv",
            index=False,
        )
